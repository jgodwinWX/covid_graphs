...
# delete the trends file if it already exists
try:
    os.remove('/home/jgodwin/python/covid/state_trends.csv')
except:
    pass
...
for state in states:
    ...
    data['weeklyHospital'] = data['hospitalizedCurrently'][::-1].rolling(window=7).mean()[::-1]
    ...
    # compute and output trend data for webpage
    case_trend = data['weeklyCases'][0] - data['weeklyCases'][14]
    pctpos_trend = data['weeklyPositivity'][0] - data['weeklyPositivity'][14]
    hosp_trend = data['weeklyHospital'][0] - data['weeklyHospital'][14]

    pos = [state,'','','']
    trendvars = [case_trend,pctpos_trend,hosp_trend]
    for a,trendvar in enumerate(trendvars):
        if trendvar > 0:
            pos[a+1] = 'up'
        elif trendvar < 0:
            pos[a+1] = 'down'
            ...
            pos[a+1] = 'missing'
    trendfile = open('/home/jgodwin/python/covid/state_trends.csv','a+',newline='')
    with trendfile:
        write = csv.writer(trendfile)
        if state == 'AL':
            write.writerows([['state','case trend','positivity trend','hospitalization trend']])
        write.writerows([pos])
